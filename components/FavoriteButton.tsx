...
import { isFavorite, addFavorite, removeFavorite, type Company } from "@/lib/favorites";
...
export default function FavoriteButton({ route, company = "KMB" }: FavoriteButtonProps) {
  const [isFav, setIsFav] = useState(false);

  useEffect(() => {
    setIsFav(isFavorite(company, route));
  }, [company, route]);

  const handleClick = () => {
    if (isFav) {
      removeFavorite(company, route);
      setIsFav(false);
    } else {
      addFavorite(company, route);
      setIsFav(true);
    }
  };
